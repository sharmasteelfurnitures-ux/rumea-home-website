...
export default function CategoryGrid() {
  ...
  return (
    <section className="py-16 md:py-24 bg-warm-offwhite">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* 6-Card Category Grid (Desktop: 3-Col Grid | Mobile: Horizontal Scroll Rail) */}
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {rooms.map((room) => (
            <Link
              key={room.id}
              href={room.href}
              className="group relative aspect-[4/3] rounded-card overflow-hidden bg-warm-alabaster border border-border-sand shadow-card hover:shadow-hover transition-all duration-300 transform hover:-translate-y-1 block"
            >
              {/* Full-bleed Photo */}
              <Image
                src={room.image}
              ...
              <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-5 flex items-end justify-between">
                ...
                <div className="w-8 h-8 rounded-full bg-white/20 backdrop-blur-xs flex items-center justify-center text-white group-hover:bg-white group-hover:text-charcoal transition-colors">
                  <ArrowRight className="w-4 h-4" />
                </div>
              </div>
            </Link>
